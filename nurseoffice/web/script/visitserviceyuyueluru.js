...
//					select += "</div>";
					...
					if ($.url.param("id") != null) {
						...
						$.ajax( {
							url : "requestVisitServiceYYByID.action",
							type : "post",
							dataType : "json",
							data : param,
...
function update_page2(result) {
	...
	for ( var one in jsonArray) {
		...
		$("input[name='lxdh']").attr("value", jsonArray[one].lxdh);
		$("input[name='jtdz']").attr("value", jsonArray[one].jtdz);
		$("input[name='ybkh']").attr("value", jsonArray[one].ybkh);
		if(jsonArray[one].kind.length==0||jsonArray[one].kind==""){
			$("select[name='fenlei']").attr("value", "0");
		}else{
			$("select[name='fenlei']").attr("value", jsonArray[one].kind);
		}
	}
}
...
function save() {
	...
	//alert(param);
	$.ajax( {
		url : "requestSaveYYVisitService.action",
		type : "post",
		dataType : "json",
		data : param,
