...
function update_page(result) {
	...
	str += "<tr bgcolor=#99ccff><td colspan=6 align=right><a href='beforepatientlist.html'>未处理</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href='afterpatientlist.html'>已处理</a></td></tr>";
	...
	for ( var one = 0; one < firstpage; one++) {
		...
		if (jsonArray[one].total.length != 0) {
			str += "<tr bgcolor="
					...
					+ color
					+ "\") onClick=mclick(this) ondblclick=\"mdbclick('"
					+ jsonArray[one].cfsb + "','" + jsonArray[one].total
					+ "','" + jsonArray[one].currents + "')\"><td>" + jsonArray[one].brxm
					+ "</td><td>";
			str += s + "</td><td>" + jsonArray[one].ksmc + "</td><td>"
					+ jsonArray[one].ysxm + "</td><td>" + jsonArray[one].kfrq
					+ "</td><td>";
...
function mdbclick(cfsb, total, currents) {
	location.href = "cfinfomx.html?patient.cfsbnumber=" + cfsb
			+ "&patient.total=" + total + "&patient.currents=" + currents + "";
}
...
function nextpage(result) {
	...
	for ( var one = from; one < to; one++) {
		...
		//alert("---------------------------"+jsonArray[0].total.length);
		if (jsonArray[one].total.length != 0) {
			str += "<tr bgcolor="
					+ color
					+ " onmouseover=changeColor1(this) onmouseout=changeColor2(this,\""
					+ color + "\") ondblclick=\"mdbclick('"
					+ jsonArray[one].cfsb + "','" + jsonArray[one].total
					+ "','" + jsonArray[one].currents + "')\"><td>" + jsonArray[one].brxm
					+ "</td><td>";
			str += s + "</td><td>" + jsonArray[one].ksmc + "</td><td>"
