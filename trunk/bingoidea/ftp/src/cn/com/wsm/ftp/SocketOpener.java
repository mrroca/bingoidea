...
public class SocketOpener
{
    ...
    public synchronized Socket makeSocket(long timeout) throws IOException
    {
        ...
        socketThread.interrupt();// socketThread.stop();
        if (ioexception != null)
            throw ioexception;
        if (socket == null)
     ...
     * System.out.print ("Opening " + address + ":" + port); System.out.println
     * (", timeout = " + timeout); long startMsec = System.currentTimeMillis();
     * try { SocketOpener opener = new SocketOpener (address, port); Socket
     * socket = opener.makeSocket (timeout); System.out.println ("Opened");
     * socket.close(); } catch (IOException e) { System.out.println (e); } long
     * elapsedMillis = System.currentTimeMillis() - startMsec;
     * System.out.println ("Elapsed time = " + elapsedMillis); }
     * 
     * 
     * /* // Test harness.
     * 
     * static public void main (String[] args) { test("www.microsoft.com", 80,
     * 0); test("www.microsoft.com", 81, 1000); }
     */
};
