...
public class UploadMedicalBa implements Job {

	private final String CONFIG_PATH = Constant.SQLSCRIPT_BA;
	...
	protected boolean init() {
		...
		mUT = new UtilTools();
		try {
			mDoc = new SAXReader().read(Constant.DBASE_CONFIG_BA);
		...
		try {
			...
			String msg = "<szwsj><hosip ip='"
					...
					+ DateFormat.getDateTimeInstance(2, 2, Locale.CHINA)
							.format(new Date()) + "] HIS病案数据采集完成:" + "success:"
					+ insert + ",error:" + error + " 耗时：" + duration + " ms</hosip></szwsj>";
			mLog.info(msg);
			rc.sendReport(msg);
		} catch (Exception e) {
			mLog.error(e.getMessage());
	...
	public static void main(String[] args) throws JobExecutionException,
			DocumentException {
		UploadMedicalBa ut = new UploadMedicalBa();
		JDBCConn.init();
		ut.execute(null);
	}
}
