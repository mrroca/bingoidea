...
public class UploadAdmission implements Job {

	private final String CONFIG_PATH = Constant.SQLSCRIPT_ZY;
	...
	public UploadAdmission() {
		...
		mCe = ConditionEntity.getInstance("MZZY");
	}
	...
	protected boolean init() {
		...
		mUT = new UtilTools();
		try {
			mDoc = new SAXReader().read(Constant.DBASE_CONFIG_ZY);
		...
		try {
			...
			String msg = "<szwsj><hosip ip='"
					...
					+ DateFormat.getDateTimeInstance(2, 2, Locale.CHINA)
							.format(new Date()) + "] HIS住院数据采集完成:" + "success:"
	...
	public static void main(String[] args) throws JobExecutionException,
			DocumentException {
		UploadAdmission ut = new UploadAdmission();
		JDBCConn.init();
		ut.execute(null);
	}
}
