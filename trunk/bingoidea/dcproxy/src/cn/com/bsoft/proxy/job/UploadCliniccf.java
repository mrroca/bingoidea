...
public class UploadCliniccf implements Job {

	private final String CONFIG_PATH = Constant.SQLSCRIPT_MZCF;
	...
	protected boolean init() {
		...
		mUT = new UtilTools();
		try {
			mDoc = new SAXReader().read(Constant.DBASE_CONFIG_MZCF);
		...
		try {
			...
			String msg = "<szwsj><hosip ip='"
					...
					+ DateFormat.getDateTimeInstance(2, 2, Locale.CHINA)
							.format(new Date()) + "] HIS处方数据采集完成:" + "success:"
	...
	public static void main(String[] args) throws JobExecutionException,
		...
		UploadCliniccf ut = new UploadCliniccf();
		JDBCConn.init();
		ut.execute(null);
	}
}
