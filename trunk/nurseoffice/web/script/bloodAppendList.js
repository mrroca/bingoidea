...
$(document)
		.ready(
				function() {
					...
					treatmentSelect += "<option value='4'>抽血</option>";
					treatmentSelect += "<option value='5'>取样</option>";
					treatmentSelect += "<option value='6'>测血糖</option>";
					treatmentSelect += "<option value='99'>其他</option>";
...
//					htm += "<td>完成人：<input name='ybkh' type='text' size='20' value='' style='width:60%;border-bottom-style: none;border-top-style: none;border-right-style: none;border-left-style: none;background-color: transparent;'></td>";
					htm += "</tr>";
					htm += "<tr>";
					htm += "<td bgcolor='#cccccc'><b> 描     述 </b></td><td colspan=5><input name='description' type='text' size='20' value='' style='width:100%;border-bottom-style: none;border-top-style: none;border-right-style: none;border-left-style: none;background-color: transparent;'></td>";
					htm += "</tr>";
					htm += "<tr>";
...
function update_addresult(data){
	...
	if (results[0].results == "success") {
		...
		$("input").attr("readonly", "readonly");
		$("input[name='submit']").attr("disabled", true);
		location.href='bloodAppendList.jsp';
	} else {
		alert("保存失败");
	}
}
