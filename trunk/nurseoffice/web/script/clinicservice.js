...
$(document).ready(function() {
	$.ajax( {
		url : "requestClinicPatientList.action",
		type : "post",
		dataType : "json",
		success : update_page
	});
...
function update_page(result) {
	// alert(result);
	var str = "<table id='clinictable'>";
	str += "<tr bgcolor=#C5E1EF><td colspan=2 style='height:30px;font-weight:bold;font-size:14px'>门诊观察列表</td></tr>";
	str += "<tr bgcolor=#cccccc><td colspan=2 style='height:30px;font-size:14px'><a href='#' onclick='clinicwpy()'><font size=2>外配</font></a></td></tr>";
...
//		alert("ooooooooo " + one);
		//alert("bbbbbbbb " + jsonArray[one].brxb);
		var s = "";
		...
		str += "<tr id='cfrow'>";
		str += "<td><a href='#' onClick=\"aclick('" + jsonArray[one].cfsb
				+ "'," + "'" + jsonArray[one].brxm + "'" + ",'"
	...
	var blfySelect = "";
	// blfySelect += "<div id='outerdiv' style='border:1px solid
	// #C0C0C0;width:82px;height:19px;clip:rect(0px,181px,18px,0px);overflow:hidden;'>";
	// blfySelect += "<div id='innerdiv' style='border:1px solid
	...
	// blfySelect += "</div>";
	var basic = "";
	basic += "<table id='cliniclurubasic'>";
	...
	basic += "<br/>";
	basic += "<br/>";
	var lurudiv = "<table id='clinicluru'>";
	...
	lurudiv += "</tr>";
	lurudiv += "<tr style='height:30px'>";
	lurudiv += "<td id='currentpatient' colspan=2></td><td colspan=2><input type='button' name='submit' style='width:92px;height:22px' value='保存' onClick='save()'><input type='hidden' name='cfsbnumber' value=''></td>";
	lurudiv += "</tr>";
	lurudiv += "</table>";
	lurudiv +="<div id='lgbr'><span><a href='clinicservicelist.html'>留观病人记录</a></span></div>";
	basic += lurudiv;
	// alert(lurudiv);
	$("#clinicresultluru").html(basic);
	$("input[name='submit']").hide();
	// $("input[name='disu']").hide();
	$("#blfynr").hide();
	$("select[name='blfy']").hide();
	$("textarea[name='chuli']").hide();
	// $("#clinicresultluru").hide();
...
//	alert("ppppppp "+cliniccurrentpage);
	cliniccurrentpage = parseInt(cliniccurrentpage);
	if (cliniccurrentpage == 1) {
		return;
	}
	from = (cliniccurrentpage - 2) * 10;
	to = (cliniccurrentpage - 1) * 10;
	if (jsonArray.length > 10) {
		clinictotalPages = jsonArray.length / 10 + "";
...
function aclick(cfsb, patientname, pid) {
	var cfsb = cfsb;
	id = pid;
	var gcjlid = pid;
//	alert("ppppp "+pid);
...
//	 alert(gcjlid);
	...
	param = "patient.cfsbnumber=" + cfsb+"&patient.gcjlid="+gcjlid;
	$.ajax( {

		url : "requestClinicPatientInfo.action",
	...
	// var disu = $("input[name='disu']").attr("value", "");
	// var guancha = $("textarea[name='guancha']").attr("value", "");
	var blfy = $("select[name='blfy']").attr("value", "");
	var chuli = $("textarea[name='chuli']").attr("value", "");
	$("input[name='submit']").show();
	// $("input[name='disu']").show();
	// $("textarea[name='guancha']").show();
	...
	// $("#clinicresultluru").show();
	$("input[name='cfsbnumber']").attr("value", cfsb);
...
function update_basic(data) {
	...
	var jsonArraylczdinfo = eval("(" + tempstr[1] + ")");
	var jsonArraygcjlinfo = eval("(" + tempstr[2] + ")");
	var lczd = "";
	...
	var qtnr = blfynr[10];
//	alert("blfynr "+blfynr);
//	alert("yyyyyyy"+fr+pz+dxy+mbch+pfsy+mgy+hxkn+xj+xom+qt);
	if(fr == 1 || fr == '1'){
...
//	alert("uuuuuuuu "+jsonArraygcjlinfo[0].username);
...
function save() {
	// alert($("input[name='cfsbnumber']").attr("value"));
	var cfsb = $("input[name='cfsbnumber']").attr("value");
	// var disu = $("input[name='disu']").attr("value");
	// var guancha = $("textarea[name='guancha']").attr("value");
	var blfy = $("select[name='blfy']").attr("value");
...
//	}
	...
	var param = "patient.cfsbnumber=" + cfsb + "&patient.blfy=" + blfy
