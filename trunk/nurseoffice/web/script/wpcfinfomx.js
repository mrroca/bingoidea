...
$(document).ready(function() {
	...
	var urlparam = $.url.param("patient.id");
		urlparam = "patient.id=" + urlparam;
		//alert(urlparam);
		$.ajax( 
			{
				url : "requestWpPatientCFMXList.action",
...
function update_page(data) {
	...
	var jsonArraynurse = eval("(" + tempstr[2] + ")");
	var cfsbnumber = $.url.param("patient.id");
	...
	for ( var one in jsonArraycfinfo) {
		...
		cflist += "<tr><td>"+jsonArraycfinfo[one].ztbs+"</td><td align=left>" + jsonArraycfinfo[one].ypmc + "</td><td>"
	...
	for ( var one in jsonArraybasicinfo) {
		...
		basic += "<tr bgcolor=#C5E1EF><td colspan=6 style='height:30px;font-weight:bold;font-size:14px'>居民基本信息</td></tr><tr>"
				...
				+ jsonArraybasicinfo[one].lczd + "</td></tr>";
	}
	basic +="</table>";
	var htm="";
	htm += basic;
	htm +="<br/><br/>";
	htm += "<table id='cfinfomxbable'><tr bgcolor=#C5E1EF><td colspan=10 style='height:30px;font-weight:bold;font-size:14px'>处方明细信息(外配)</td></tr>";
	htm += cflist;
	var m = 0;
	...
	var paramurl = $.url.param("patient.id");
	...
	$.ajax( {
		...
		// 处理程序
		url : "requestWPPatientcurrents.action",
		...
		success : function(data){
				...
				if(null!=jsonArraycurrents[0]){
					...
					$("input[name='current']").attr("value",0);
				}
			}
...
//	}
	...
	if (!$("input[name='inputtotal']").attr("disabled")
		...
		var param = "patient.optid=" + user + "&patient.cfsbnumber="
				...
				+ "&patient.currents=" + current + "";
		$.ajax( {

			url : "saveWPPatientInfo.action",
