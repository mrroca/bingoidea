...
//					htm += "</tr>";
					htm += "<tr>";
					htm += "<td style='magin:0px;padding:0px;width:20%;' bgcolor='#cccccc'><b>*操作事项</b></td><td><div id='selectbox'><div id='sexlabel'></div>"
							+ contentselect + "</div></td>";
					htm += "<td bgcolor='#cccccc'><b>完成情况</b></td><td colspan=2><input name='sanitationcomplete' type='text' size='20' style='width:100%;border-bottom-style: none;border-top-style: none;border-right-style: none;border-left-style: none;background-color: transparent;'></td>";
					htm += "</tr>";
//					htm += "<tr>";
//					htm += "<td colspan=4>完成情况：<textarea name='sanitationcomplete' cols ='100%' rows='5' style='border:none;overflow:auto'></textarea></td>";
//					htm += "</tr>";
					htm += "<tr>";
					htm += "<td bgcolor='#cccccc'><b>操作时间</b></td><td><input name='time' type='text' size='20' value='"+nowDate.toLocaleDateString().split(" ", 1)+"' style='width:100%;border-bottom-style: none;border-top-style: none;border-right-style: none;border-left-style: none;background-color: transparent;'></td>";
					htm += "<td bgcolor='#cccccc'><b>操作人</b></td><td align='left'><span id='opt'><span></td><td><input type='button' name='submit' value='保存' style='width:102px;height:32px' onClick='save()'></td>";
					htm += "</tr>";
					htm += "</table>";
//					var wholehtm = "";
//					wholehtm += "<form>";
...
//					$("textarea[name='sanitationcontent']").attr("value", "请输入操作事项");
//					$("textarea[name='sanitationcontent']").focus(function() {
//						if ($(this).attr("value") == "请输入操作事项") {
...
//	var user = $("select[name='username']:checked").val();
...
//	param += "&patient.time=" + time;
	param += "&patient.optid=" + user;
	$.ajax( {
		url : "requestSaveSanitationService.action",
		type : "post",
		dataType : "json",
		data : param,
...
//	alert("results:  "+results[0].results);
