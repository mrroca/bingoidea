...
public class UploadClinicjb implements Job {

	private final String CONFIG_PATH = Constant.SQLSCRIPT_MZJB;
	...
	protected boolean init() {
		...
		mUT = new UtilTools();
		try {
			mDoc = new SAXReader().read(Constant.DBASE_CONFIG_MZJB);
		...
		try {
			...
			String msg = "<szwsj><hosip ip='"
					...
					+ DateFormat.getDateTimeInstance(2, 2, Locale.CHINA)
							.format(new Date()) + "] HIS疾病数据采集完成:" + "success:"
	...
	public static void main(String[] args) throws JobExecutionException,
			DocumentException {
		UploadClinicjb ut = new UploadClinicjb();
		JDBCConn.init();
		ut.execute(null);
	}
}
