...
public class UploadClinicks implements Job {

	private final String CONFIG_PATH = Constant.SQLSCRIPT_MZ;
	...
	protected boolean init() {
		...
		mUT = new UtilTools();
		try {
			mDoc = new SAXReader().read(Constant.DBASE_CONFIG_MZ);
		...
		try {
			...
			String msg = "<szwsj><hosip ip='"
					...
					+ DateFormat.getDateTimeInstance(2, 2, Locale.CHINA)
							.format(new Date()) + "] HIS门诊数据采集完成:" + "success:"
	...
	public static void main(String[] args) throws JobExecutionException,
			DocumentException {
		UploadClinicks ut = new UploadClinicks();
		JDBCConn.init();
		ut.execute(null);
	}
}
