...
public class UploadCheck implements Job {

	private final String CONFIG_PATH = Constant.SQLSCRIPT_CHECK;
	...
	public UploadCheck() {
		...
		mCe = ConditionEntity.getInstance("CHECK");
	}
	...
	protected boolean init() {
		...
		mUT = new UtilTools();
		try {
			mDoc = new SAXReader().read(Constant.DBASE_CONFIG_CHECK);
		...
		try {
			...
			String msg = "<szwsj><hosip ip='"
					...
					+ DateFormat.getDateTimeInstance(2, 2, Locale.CHINA)
							.format(new Date()) + "] RIS检查数据采集完成:" + "success:"
	...
	public static void main(String[] args) throws JobExecutionException,
			DocumentException {
		UploadCheck ut = new UploadCheck();
		JDBCConn.init();
		ut.execute(null);
	}
}
