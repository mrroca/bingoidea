...
/**
 * The class that implement this interface have the ability to monitor the
 * progress of upload and download files in the FtpBean.<br>
 * You can pass the object which implement this interface to some put/get
 * methods of the FtpBean object. So that when there are any bytes read from the
 * server, the byteRead(int) method of the object you passed is invoked. And the
 * byteWrite(int) method is invoked when any bytes is written to the server
 * side.<br>
 * A sample code is like this: <font size="2">
 * 
 * <pre>
...
{
    /**
     * This method is called every time new bytes are read in downloading
     * process.
     * 
     * @param bytes
     *            The number of new bytes read from the server.
     */
    void byteRead(int bytes);


    /**
     * This method is called every time new bytes is written to the ftp server
     * in uploading process.
     * 
     * @param bytes
     *            The number of new bytes write to the server.
     */
