 ...
 * You can use this bean in some visual development tools (eg. IBM VA) or just
 * include its files and use its methods.<br>
 * This class is thread safe, only one thread can acquire the object and do some
 * ftp operation at a time.
 * <p>
 ...
 * <font color="#0000ff"> ftp.ftpConnect("servername", "username", "password");
 * </font><br>
 * // Some methods return null if you call them without establish connection
 * first. <br>
 ...
 * <p>
 * Remarks:<br>
 * 1) Whenever a <b>ftp command failed</b> (eg. Permission denied), the methods
 * throw an FtpException.<br>
 * So you need to catch the FtpException wherever you invoke those methods of
 * this class.<br>
 * 2) This bean use <b>passive mode</b> to establish data connection by
 * <b>default</b>. If this cause problem from firewall of the network, try using
 * active mode:<br>
 * ftp.setPassiveModeTransfer(false);<br>
 * 3) To turn on <b>debug mode</b>, you need to change the source of this class.
 * Then re-compile it.<br>
 * 4) For <b>timeout on creating Socket</b>. if a timeout is being set and
 * operation timeout, a java.io.InterruptedIOException is throw. This is the
 * case for both passive transfer mode and establishment of connection to the
 * server at the beginning. For active transfer mode, timeout is set in the
 * servers ftpd. If there is timeout, the servers ftp software return an error
 ...
 * please don't include the user name and password in the source code of your
 * applet. As anyone who can get your class files can get your user name and
 * password. It is reasonable to ask the user for user name and password if you
 * are going to use FTP in the applet.<br>
 * 2) If you use it in an applet, please be aware of the security restriction
 * from the browser. As an unsigned applet can ONLY connect to the host which
 * serves it. Also, some methods in this bean will write/read to the local file
 * system. These methods are also restricted by the browser.<br>
 * <br>
 * 
 * If you find any bugs in this bean or any comment, please give me a notice at<br>
 * <a href="mailto:calvin_tai2000@yahoo.com.hk">Calvin(calvin_tai2000@yahoo.com.
 * hk)</a><br>
 * 
 */
...
public class FtpBean
{
    ...
    protected final static String REPLY_POS_PRE = "1"; // Positive Preliminary
                                                       // Reply
    protected final static String REPLY_POS_CMP = "2"; // Positive Completion
                                                       // Reply
    protected final static String REPLY_POS_INT = "3"; // Positive Intermediate
                                                       // Reply
    protected final static String REPLY_TRA_NEG = "4"; // Transient Negative
                                                       // Completion Reply
    protected final static String REPLY_PER_NEG = "5"; // Permanent Negative
                                                       // Completion Reply
    protected final static String REPLY_UNDEF = "0"; // Undefined reply, should
                                                     // not be exist.
    ...
    private int port = 2121; // FTP port number, default 21
    ...
    private Vector<Thread> thread_spool = new Vector<Thread>(); // Spool for the
                                                                // waiting
                                                                // threads
     ...
     */
    public void ftpConnect(String server, String user) throws IOException,
        FtpException
    {
     ...
     */
    public void ftpConnect(String server, String user, String password,
        String acct) throws IOException, FtpException
    {
        ...
        try
        {
            ...
            {
                /*
                 * If a timeout has been set before opening the socket, use the
                 * SocketOpener. Note that if the thread times out, it is
                 * forcibly terminated.
                 */
            ...
            in = new BufferedReader(new InputStreamReader(socket
                .getInputStream(), FTP_ENCODING));
            out = new PrintWriter(new OutputStreamWriter(socket
                .getOutputStream(), FTP_ENCODING), true);
     ...
     * @exception FtpException
     *                if a ftp error occur. (eg. A file named the new file name
     ...
     *            "\n", "\r")
     * @return The Ascii content of the file. It uses parameter 'separator' as
     *         the line separator.
     * @exception FtpException
     *                if a ftp error occur. (eg. no such file in this case)
     * @exception IOException
     ...
     */
    public String getAsciiFile(String filename, String separator,
        FtpObserver observer) throws IOException, FtpException
    {
        if (out == null)
            return null;
        ...
        try
        {
            ...
            str_content = new String(getFile(filename, 0, observer),
                FTP_ENCODING);
        }
        finally
        {
    ...
    public void getAsciiFile(String ftpfile, String localfile,
        String separator, FtpObserver observer) throws IOException,
        FtpException
    {
        final int BUF = 1024;
        if (out == null)
        ...
        try
        {
            ...
            BufferedInputStream reader = new BufferedInputStream(sock
                .getInputStream());
            ...
            while ((offset = reader.read(data, 0, BUF)) != -1)
            {
                ...
                // the
                // line separactor in ASCII transmission. Then it can be
                // replaced.
     ...
     * convert the NVT-ASCII format line separator into its system line
     * separator.
     * 
     * @param filename
     *            The name of file
    ...
    public void appendAsciiFile(String filename, String content,
        String separator) throws IOException, FtpException
    {
        if (out == null)
            return;
     ...
     * 
     * @param filename
     *            The name of the binary file to be got.
    ...
    /**
     * Get a binary file at a restarting point. Return null if restarting point
     * is less than zero.
     * 
     * @param filename
    ...
    /**
     * Read file from ftp server and write to a file in local hard disk. This
     * method is much faster than those method which return a byte array<br>
     ...
     * 
     * @param ftpfile
     *            Name of file to be get from the ftp server, can be in full
     *            path.
     * @param localfile
     *            Name of local file to be write, can be in full path.
     * @exception FtpException
     *                if a ftp error occur. (eg. No such file in this case)
     * @exception IOException
     *                if an I/O error occur.
    ...
    /**
     * Read from a ftp file and restart at a specific point. This method is much
     * faster than those method which return a byte array<br>
     * if the network is fast enough.<br>
     * Remark:<br>
     * Cannot be used in unsigned applet.
     * 
     ...
     *            The name of file.
     * @param content
     *            The byte array to be written to the server.
     * @exception FtpException
     *                if a ftp error occur. (eg. permission denied in this case)
     * @exception IOException
     *                if an I/O error occur.
    ...
    /**
     * Put a binary file to the server from an array of byte with a restarting
     * point
     * 
     * @param filename
     *            The name of file.
    ...
    /**
     * Read a file from local hard disk and write to the server. <br>
     * Remark:<br>
     * <br>
     * Cannot be used in unsigned applet.
     * 
    ...
    /**
     * Read a file from local hard disk and write to the server with restarting
     * point. Remark:<br>
     * Cannot be used in unsigned applet.
     * 
     * @param local_file
     ...
     *            Name of file in the ftp server, can be in full path.
     * @param restart
     *            The restarting point, ignored if less than or greater than
     *            zero.
     * @exception FtpException
     *                if a ftp error occur. (eg. permission denied)
     * @exception IOException
     ...
     */
    public void putBinaryFile(String local_file, String remote_file,
        long restart) throws IOException, FtpException
    {
        if (out == null)
            return;
     ...
     */
    public void putBinaryFile(String local_file, String remote_file,
        long restart, FtpObserver observer) throws IOException, FtpException
    {
        if (out == null)
            return;
    ...
    /**
     * Read a file from local hard disk and append a file on the server. Remark:<br>
     * Cannot be used in unsigned applet.
     * 
     * @param filename
     *            Name of local file and remote file.
     * @exception FtpException
     *                if a ftp error occur. (eg. permission denied)
     * @exception IOException
     *                if an I/O error occur.
     */
    public void appendBinaryFile(String filename) throws IOException,
        FtpException
    {
    ...
    /**
     * Read a file from local hard disk and append to the server with restarting
     * point. Remark:<br>
     * Cannot be used in unsigned applet.
     * 
     * @param local_file
     ...
     *                if an I/O error occur.
     */
    public String getDirectoryContentAsString() throws IOException,
        FtpException
    {
        if (out == null)
            return null;
        ...
        try
        {
            ...
            BufferedReader listen = new BufferedReader(new InputStreamReader(
     ...
     *                if an I/O error occur.
     */
    public void removeDirectory(String directory) throws IOException,
        FtpException
    {
        if (out == null)
            return;
    ...
    /**
     * Get the type of operating system of the server. Return null if it is not
     * currently connected to any ftp server.
     * 
    ...
    {
        acquire(); // Acquire the object
        pcs.firePropertyChange("port", new Integer(this.port),
            new Integer(port));
        this.port = port;
        release(); // Release the object
    }
    ...
    /**
     * Set timeout when creating a socket. This include trying to connect to the
     * ftp server at the beginnig and trying to connect to the server in order
    ...
    {
        acquire(); // Acquire the object
        pcs.firePropertyChange("socketTimeout", new Integer(this.timeout),
            new Integer(timeout));
    ...
    /**
     * Return the account information. Return "" if it is not connected to any
     * server.
     */
     ...
     * 
     * @return Reply of the last comomand<br>
     ...
     * 
     * @return Reply message of the last command<br>
     *         for example:<br>
     *         250-Please read the file README<br>
     *         250-it was last modified on Wed Feb 10 21:51:00 1999 - 268 days
     *         ago
     */
    ...
    public void setPassiveModeTransfer(boolean passive)
    {
        ...
        pcs.firePropertyChange("passiveModeTransfer",
            new Boolean(this.passive), new Boolean(passive));
    ...
    private String getReplyType(String reply_code)
    {
        ...
        if (!(reply_type.equals(REPLY_POS_PRE)
                ...
                || reply_type.equals(REPLY_TRA_NEG) || reply_type
            .equals(REPLY_PER_NEG)))
            return REPLY_UNDEF;
        return reply_type;
    }
    ...
    /*
     * Read the respond message from the server's inputstream and assign to
     * replymessage
     */
    private void getRespond(String cmd) throws IOException, FtpException
    {
        ...
        {
            throw new FtpException("Valid reply for command '" + cmd
                    + "' not found in reply resource bundle");
        }
        boolean valid = false;
     ...
     */
    private void ftpCommand(String cmd, String param) throws IOException,
        FtpException
    {
        if (out == null)
            return;
    ...
    /*
     * Read bytes continuously from the BufferedInputStream. Add the new read
     * bytes to an byte array. Return the byte array after all bytes are read.
     * If the FtpObserver is not null, it invokes its byteRead method every time
     * there are new bytes read.
     */
    ...
    /*
     * Establish data connection in passive mode using "PASV" command Change the
     * server to passive mode. by the command "PASV", it will return its address
     * and port number that it will listen to. Create a Socket object to that
     * address and port number. Then return the Socket object.
     */
    private Socket getPassiveDataSocket() throws IOException, FtpException
    {
        ...
        String SRV_IP = address[0] + '.' + address[1] + '.' + address[2] + '.'
                + address[3];
        ...
        if (DEBUG)
            System.out.println("FtpBean: Extracted Server ip - " + SRV_IP
    ...
    /*
     * Establish data connection in active mode using "PORT" command. It create
     * a ServerSocket object to listen for a port number in local machine. Use
     * port command to tell the server which port the local machine is
     * listenning. Return the ServerSocket object.
     */
    private ServerSocket getActiveDataSocket() throws IOException, FtpException
    {
        ...
        ServerSocket ssocket = new ServerSocket(0); // ServerSocket to listen to
                                                    // a random free port number

        int local_port = ssocket.getLocalPort(); // The port number it is
                                                 // listenning to
    ...
    {
        pcs.firePropertyChange("reply", this.reply, reply);
        this.reply = reply;
    }
    ...
    {
        pcs.firePropertyChange("replyMessage", this.replymessage, replymessage);
    ...
    {
        pcs.firePropertyChange("acctInfo", this.acctInfo, acctInfo);
    ...
    {
        pcs.firePropertyChange("serverName", this.server, server);
        this.server = server;
    }
    ...
    {
        pcs.firePropertyChange("userName", this.user, user);
        this.user = user;
    }
     ...
     */
    private void setTransferType(boolean ascii) throws IOException,
        FtpException
    {
        if (ascii)
        {
            ftpCommand(CMD_TYPE, TF_MOD_ASCII);
        }
        else
        {
            ftpCommand(CMD_TYPE, TF_MOD_BIN);
        }
    }
     ...
     */
    private String changeLineSeparator(String text, String old_separator,
    ...
    /*
     * Read the data from the BufferedInputStream object and write to the
     * RandomAccessFile object
     */
    private void readData(BufferedInputStream reader, RandomAccessFile out,
        FtpObserver observer) throws IOException
    {
        int offset;
        byte[] data = new byte[1024];
        while ((offset = reader.read(data)) != -1)
     ...
     */
    private void writeData(RandomAccessFile din, DataOutputStream dout,
        FtpObserver observer) throws IOException
    {
        int offset;
        byte[] data = new byte[1024];
    ...
    // All of this thread safe operation are transparent to the programmers who
    // are using this bean.
    // All threads that want to do some ftp operation must acquire this object
    // first.
    // Then release the object after those operation or an exception is throwed.
    // When the object is acquired by a thread, other threads that want to do
    // ftp operation,
    // will be placed in the thread_spool. Then have rights to access this
    // thread when the
    ...
    // }
    //
    // This can ensure the thread will release the object even an exception is
    // threw.

    /*
     * Acquire this FtpBean object. If there is a thread acquired this object
     * already. Put itself into the thread spool.
     */
    private void acquire()
    {
        ...
        synchronized (lock)
        {
            ...
            if (DEBUG) // Debug message
                System.out.println("Add thread to spool, size: "
                        + thread_spool.size());
        }
        try
        {
        ...
        /*
         * Old locking synchronized(lock) {
         * 
         * // Loop if object is acquired by a thread // or there are other
         * threads waiting in the thread_spool loop: while(acquired ||
         * thread_spool.size() > 0) { if(thread_spool.contains(thread)) {
         * if(thread_spool.elementAt(0).equals(thread) && !acquired) { // Object
         * is released by previous thread // And this is the first thread in the
         * thread_spool // Then break the loop.
         * thread_spool.removeElement(thread); break loop; } else lock.notify();
         * // Notify other threads } else thread_spool.addElement(thread); //
         * Add thread to thread_spool // Go to wait try { lock.wait(); }
         * catch(Exception e) { System.out.println(e); }
         * 
         * } // Acquire this object acquired= true;
         * 
         * if(DEBUG) // Debug message
         * System.out.println("FtpBean: Acquired by thread."); }
         */
    ...
    private void release()
    {
        ...
        /*
         * synchronized(lock) { if(DEBUG) // Debug message
         * System.out.println("FtpBean: Released by thread.");
         * 
         * // Release this object acquired = false;
         * 
         * // Notify other threads if(thread_spool.size() > 0) lock.notify();
         * else lock.notifyAll(); }
         */
    }
}
