...
/**
 * This class is used to parse the information of a returned String from the
 * 'LIST' command.<br>
 ...
 * 7) Size
 * <p>
 * Sample code for using this class:<br>
 * <b> while(ftplist.next())<br>
 * {<br>
 * System.out.println(ftplist.getName());<br>
 * }</b><br>
 * <br>
 * Normally, we can see two formats returned by the ftp LIST command.<br>
 * One is the standard UNIX format, and another one is the MS-DOS format (rarely
 * see).<br>
 * This class can parse both formats. Unfortunely, the following information are
 * missed in<br>
...
public class FtpListResult
{
    ...
    public final static int DIRECTORY = 1, FILE = 2, LINK = 3, BLK_DEV = 4,
            CHAR_DEV = 5, OTHERS = 6;
    ...
    /**
     * A FtpListResult is initially positioned before its first row. the first
     * call to next makes the first row the current row; the second call makes
    ...
    protected void parseList(String strlist, String system_type)
    {
        ...
        {
            System.out.println("FtpListResult: Remote system type is "
                    + system_type);
        ...
        if (strlist.length() <= 0)
        {
            if (DEBUG) // Debug message
                System.out
                    .println("FtpListResult: Length of String == 0, return.");
            return;
        }
    ...
    /*
     * Parse a UNIX format list. With skipping number of lines. The columns of a
     * typical UNIX format list is like: drwxr-xr-x 6 appli appli 1024 May 17
     * 20:33 java -rw------- 1 appli appli 1005 May 1 21:21 mp3list drwxr-xr-x 5
     * appli appli 1024 May 17 16:56 perl drwxr-xr-x 6 appli appli 1024 Apr 30
     * 23:18 public_html
     * 
    ...
    private void parseUnixList(String strlist, int skip)
    {
        ...
        if (DEBUG) // Debug message
            System.out
                .println("FtpListResult: Parse UNIX format list ...... skip "
                        + skip + " line");
        ...
        for (int i = 0; i < num_lines; i++)
        {
            ...
            // Some UNIX format list doesn't contain the column for group like
            // this:
            ...
            // So a NumberFormatException is threw as it try to convert words
            // like
            ...
            catch (NumberFormatException e)
            {
                ...
                {
                    size[i] = Long.parseLong(skip_col);
                }
                date[i] = temp.concat(" ");
                group[i] = "";
                date_cols = 2;
    ...
    private int countLine(String str)
    {
        ...
        if (DEBUG) // Debug message
            System.out.println("FtpListResult: " + num_of_lines
